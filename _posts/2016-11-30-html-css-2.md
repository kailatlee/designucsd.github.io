...
Awesome! We've added our hero content, but it could really use a bit of styling. Let's first create a new file in our current directory and call it `stylesheet.css`. Don't forget, we have to also link our HTML page to our newly created css file. You can do that by using the `<link>` tag in the `<head>` section of `index.html`.

Once you're ready, let's start by adding removing default browser spacing and set up our font. Just like in our last workshop, we'll be using Lato.
...
Time to add color! We can select the outermost `<div>` using the css selector `.hero` and change it's `color` and `background-color` properties

```css
.hero {
   color: white;
   background-color: #112333; /* Hexcode for navy blue */
}
```
...
Now that we have some color, lets add some spacing and align our text to the center in our `.hero-content` `<div>`
...
From here, we can add a bit of font styling to our `.title` and `.subtitle`.
...
Last but not least, lets give the our `.button` some styling
...
* `div` - **An outer div to wrap the entire Info section.** Let's use class='info'
  * `div` - **An inner div to wrap our first half of content** Let's use class='half'
    * `div` - **Another div to wrap its contents** Let's use class='half-content'
      * `h2` - **For our main header.**
      * `p` - **A paragraph.** A class='subtitle' sounds good
      * `p` - **Another paragraph.** We'll make this italic later on using class='italic'
  * `div` - **An inner div to wrap our second half of content** Let's use class='half'
    * `div` - **Another div to wrap its contents** Let's use class='half-content'
      * `img` - **For our image.**
...
Take a look at our HTML and brainstorm some ways we can split our two `.half` classes. We know each `div` takes about half of the page, so we should set our `width: 50%`. Additionally, this would be a great place to use our `display: inline-block` property since we want our divs to be inline with each other, and its inner content to be block elements.
...
```
* `width: 50%`: For width, we can you relative % values. This is saying that the width should take up 50% of the first parent div, which in our case is `.info`, that has a width defined.
...
<div class="nav">
    <div class="nav-content">
        <div class="nav-left">
            <span class="logo">envision</span>
        </div>
        <ul class="nav-right nav-list">
            <li>Tour</li>
            <li>Customers</li>
            <li>New Features</li>
            <li>Enterprise</li>
